...
async function installFromBundle(
  ...
  try {
    ...
    reportProgress(
      deps,
      `Downloading ${selectedBundle.profile} bundle for ${librarySlug(namespace, name)}@${version}...`,
    );
  ...
  try {
    ...
    if (selectedBundle) {
      try {
        ...
        reportProgress(
          deps,
          `Bundle install failed for ${librarySlug(namespace, name)}@${version}; falling back to page API...`,
        );
...
export async function searchDocs(deps: AppDeps, input: SearchDocsInput): Promise<CommandResult> {
  ...
  if (input.library) {
    ...
    if (!requestedInstalled) {
      return errorResult(
        input.version ? `${input.library}@${input.version} is not installed.` : `${input.library} is not installed.`,
        "NOT_INSTALLED",
        {
          library: input.library,
...
export async function updateDocs(deps: AppDeps, input: { library?: string }): Promise<CommandResult> {
  ...
  if (targets.length === 0) {
    return structuredTextResult(
      input.library ? `${input.library} is not installed. Use install_docs first.` : "No documentation packages installed.",
      { results: [] },
    );
  }
  ...
  for (const lib of targets) {
    try {
      ...
      messages.push(`${librarySlug(lib.namespace, lib.name)}: update failed — ${(error as Error).message}`);
      results.push({
        library: librarySlug(lib.namespace, lib.name),
        version: lib.version,
        status: "failed",
